...
public:
  ...
  void simpleAPITest() {
    ...
    Cornu::Debugging::get()->printf(
        "SimpleAPI Finished, #lines = %d, #arcs = %d, #clothoids = %d\n",
        nPrims[0], nPrims[1], nPrims[2]);

    std::vector<Cornu::BasicBezier> bezier = Cornu::toBezierSpline(result, 1.);

    Cornu::Debugging::get()->printf(
        "Conversion to Bezier results in %d segments\n", bezier.size());
  }
  ...
  void fullAPITest() {
    ...
    // initialize the curve to a sequence of points (there will usually be more
    // and they'll come from a mouse or tablet)
    ...
    Cornu::Debugging::get()->printf(
        "FullAPI Finished, #lines = %d, #arcs = %d, #clothoids = %d\n",
        nPrims[0], nPrims[1], nPrims[2]);
    ...
    Cornu::Debugging::get()->printf(
        "Small Pixels Finished, #lines = %d, #arcs = %d, #clothoids = %d\n",
        nPrims[0], nPrims[1], nPrims[2]);

    // output is destroyed with the destruction of the smart pointer and the
    // fitter
  }
};
