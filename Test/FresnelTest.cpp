...
public:
  ...
  void run() {
    ...
    //
    // ArrayXd diffV = (s1 - s2).array().square() + (c1 -
    // c2).array().square(); ArrayXd errVec =
    //     (diffV / (s1.array().square() + c1.array().square())).sqrt();
    // double relErr = errVec.maxCoeff();
    // Debugging::get()->printf("Done, max relative error = %.10lf", relErr);
    //
    // CORNU_ASSERT_LT_MSG(relErr, 1e-6, "Max relative error is " << relErr);
    // CORNU_ASSERT(relErr < 1e-6);
  }

  double f(int i, int num) { return (double(i) / double(num)) * 10. - 5.; }
};
