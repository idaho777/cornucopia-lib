...
public:
  ...
  void run() {
    ...
    // for (int i = 0; i < 200; ++i) {
    //   ClothoidPtr clothoid =
    //       new Clothoid(Vector2d(0.5, 1), 0.3, drand(0.01, 3.),
    //       drand(-5.1, 5.1),
    //                    drand(-5.1, 5.1));
  ...
  void testProject(CurvePtr curve) {
    ...
    for (int i = 0; i < 1000; ++i) {
      ...
      if (!curve->isClosed() || fabs(s - newS) + tol < curve->length())
        CORNU_ASSERT_LT_MSG(fabs(s - newS), tol,
                            "Projecting again should yield the same result");

      double len = curve->length();
      for (double newS = 0; newS < len + 1e-8; newS += (len / 30.)) {
        Vector2d newPt = curve->pos(newS);
        CORNU_ASSERT_LT_MSG(
            projDist, (pt - newPt).norm() + tol * (pt - newPt).squaredNorm(),
            "Projection should be closest point");
      }
    }
    if (false)
