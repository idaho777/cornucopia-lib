...
public:
  static DebuggingTestImpl *get() {
    ...
    DebuggingTestImpl *cur =
        dynamic_cast<DebuggingTestImpl *>(Cornu::Debugging::get());
    if (cur)
      return cur;
...
struct TestComparator {
  bool operator()(TestCase *t1, TestCase *t2) const {
    return t1->name() < t2->name();
  }
};

int main() {
  std::sort(TestCase::allTests().begin(), TestCase::allTests().end(),
            TestComparator());
