...
ParamWidget::ParamWidget(QWidget *parent) : QDockWidget(parent) {
  ...
  connect(_ui->presetList, SIGNAL(itemSelectionChanged()), this,
          SLOT(setPresetFromList()));

  // user parameters
  for (int i = 0; i < Cornu::Parameters::INTERNAL_PARAMETERS_MARKER; ++i) {
    _ui->paramLayout->insertWidget(
        _ui->paramLayout->count() - 1,
        new OneParamWidget(this, _ui->paramScrollContents,
                           Cornu::Parameters::parameters()[i]));
  }
  ...
  for (int i = 0; i < Cornu::NUM_ALGORITHM_STAGES; ++i) {
    ...
    AlgorithmChoiceWidget *choiceWidget =
        new AlgorithmChoiceWidget(this, _ui->algScrollContents, stage);
    _ui->algLayout->insertWidget(_ui->algLayout->count() - 1, choiceWidget);
  }

  // internal parameters
  for (int i = Cornu::Parameters::INTERNAL_PARAMETERS_MARKER + 1;
       i < (int)Cornu::Parameters::parameters().size(); ++i) {
    _ui->internLayout->insertWidget(
        _ui->internLayout->count() - 1,
        new OneParamWidget(this, _ui->internScrollContents,
                           Cornu::Parameters::parameters()[i]));
  }
...
ParamWidget::~ParamWidget() { delete _ui; }

void ParamWidget::_addPreset(const Cornu::Parameters &params) {
  _presets.push_back(params);
  QListWidgetItem *item =
      new QListWidgetItem(params.name().c_str(), _ui->presetList);
}

void ParamWidget::setParameter(Cornu::Parameters::ParameterType param,
                               double value) {
...
void ParamWidget::setPresetFromList() {
  for (int i = 0; i < (int)_presets.size(); ++i) {
    if (_ui->presetList->item(i)->isSelected()) {
...
void ParamWidget::makePreset() {
  QString presetName =
      QInputDialog::getText(this, "Make Preset", "Enter the preset name");
...
void ParamWidget::_parametersChanged() {
  ...
  for (int i = 0; i < (int)_presets.size(); ++i) {
    if (_presets[i] == _parameters) {
      ...
      _ui->presetList->item(i)->setSelected(false);
    }
  }
