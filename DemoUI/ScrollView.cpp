...
void ScrollView::resizeEvent(QResizeEvent *) { update(); }
...
void ScrollView::update() {
  ...
  QRect sceneRect = scene2view.mapRect(sceneRectF)
                        .toAlignedRect(); // scene rect in view coordinates
...
void ScrollView::mouseMoveEvent(QMouseEvent *e) {
  ...
  int dx = e->position().x() - _prevMousePos.x();
  int dy = e->position().y() - _prevMousePos.y();
  _prevMousePos = e->pos();
...
void ScrollView::wheelEvent(QWheelEvent *e) {
  ...
  _zoom *= pow(zoomFactor, e->pixelDelta().y());
