...
void DebuggingImpl::elapsedTime(const string &description) {
  ...
  printf("Timing: %20s  ---  %.3lf", description.c_str(),
...
void DebuggingImpl::drawPoint(const Vector2d &pos, const Color &color,
                              const std::string &group) {
  if (_scene)
    _scene->addItem(new PointSceneItem(
        pos, group.c_str(), QColor::fromRgbF(color[0], color[1], color[2])));
}

void DebuggingImpl::drawLine(const Vector2d &p1, const Vector2d &p2,
                             const Color &color, const std::string &group,
                             double thickness, LineStyle style) {
  if (_scene)
    _scene->addItem(new LineSceneItem(p1, p2, group.c_str(),
                                      _toQPen(color, thickness, style)));
}

void DebuggingImpl::drawCurve(Cornu::CurveConstPtr curve, const Color &color,
                              const std::string &group, double thickness,
                              LineStyle style) {
  if (_scene)
    _scene->addItem(new CurveSceneItem(curve, group.c_str(),
                                       _toQPen(color, thickness, style)));
}

void DebuggingImpl::drawCurvatureField(Cornu::CurveConstPtr curve,
                                       const Color &color,
                                       const std::string &group,
                                       double thickness, LineStyle style) {
  if (!_scene)
  ...
  for (double x = 0; x < curve->length() + step * 0.5; x += step) {
    ...
    _scene->addItem(new LineSceneItem(pos, pos - norm * length, group.c_str(),
                                      _toQPen(color, thickness, style)));
  }
}

QPen DebuggingImpl::_toQPen(const Color &color, double thickness,
                            LineStyle style) {
