...
OneParamWidget::OneParamWidget(ParamWidget *paramWidget, QWidget *parent,
                               const Cornu::Parameters::Parameter &param)
    : _paramWidget(paramWidget), QWidget(parent), _param(param),
      _changing(false), _value(param.defaultVal), _infinity(false) {
  _ui = new Ui::OneParamWidgetUi();
  _ui->setupUi(this);

  _ui->label->setText(_param.typeName.c_str());

  connect(_ui->spinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setValue(double)));
  connect(this, SIGNAL(spinBoxValueChanged(double)), _ui->spinBox,
          SLOT(setValue(double)));
  connect(_ui->slider, SIGNAL(valueChanged(int)), this,
          SLOT(setSliderValue(int)));
  connect(this, SIGNAL(sliderValueChanged(int)), _ui->slider,
          SLOT(setValue(int)));
  connect(_ui->infinityBox, SIGNAL(toggled(bool)), this,
          SLOT(setInfinity(bool)));
  connect(this, SIGNAL(infinitySet(bool)), _ui->infinityBox,
  ...
  _ui->spinBox->setRange(
      0, 1e10); // TODO: maximum value slider allows without overflow
  ...
  if (param.defaultVal == Cornu::Parameters::infinity)
    setValue(param.min); // make sure the parameter UI has a default
                         // non-infinite value
...
int OneParamWidget::_toSlider(double val) {
  return min(_sliderMarks,
             max(0, (int)floor(0.5 + _sliderMarks * (val - _param.min) /
                                         (_param.max - _param.min))));
}
...
ParameterSetter::ParameterSetter(ParamWidget *paramWidget,
                                 OneParamWidget *oneParamWidget)
    : QObject(oneParamWidget) {
  _parameter = oneParamWidget->parameter().type;
  connect(oneParamWidget, SIGNAL(valueChanged(double)), this,
          SLOT(valueChanged(double)));
  connect(this, SIGNAL(setParameter(Cornu::Parameters::ParameterType, double)),
          paramWidget,
          SLOT(setParameter(Cornu::Parameters::ParameterType, double)));
}
