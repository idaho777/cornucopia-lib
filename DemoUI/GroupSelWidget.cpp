...
VisibilitySetter::VisibilitySetter(QCheckBox *parent, ScrollScene *scene,
                                   QString group)
    : QObject(parent), _scene(scene), _group(group) {
  connect(parent, SIGNAL(stateChanged(int)), this,
...
void GroupSelWidget::sceneChanged() {
  ...
  QList<QString> groups = groupSet.values();
  ...
  for (int i = 0; i < (int)groups.size(); ++i) {
    ...
    box->setFont(font);
    box->setCheckState(_scene->isGroupVisible(groups[i]) ? Qt::Checked
                                                         : Qt::Unchecked);
