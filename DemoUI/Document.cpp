...
void Document::curveDrawn(Cornu::PolylineConstPtr polyline) {
  ...
  const double threshold =
      Cornu::SQR(sketch.params.get(Cornu::Parameters::OVERSKETCH_THRESHOLD));
...
void Document::selectAll() {
  for (int i = 0; i < (int)_sketches.size(); ++i)
    _sketches[i].selected =
        _sketches[i]
            .sceneItem; // only select sketches that have something visible
  _selectionChanged();
}

void Document::deleteItem() {
  // select oversketched stuff for deletion
  for (int i = (int)_sketches.size() - 1; i > 0;
       --i) // proceed in reverse order so we get everything
  {
...
void Document::open() { _readFile("Open Curve", true); }

void Document::insert() { _readFile("Insert Curve", false); }

void Document::selectAt(const Eigen::Vector2d &point, bool shift,
                        double radius) {
  if (!shift)
    clearSelection();
...
void Document::_processSketch(int idx) {
  ...
  if (_sketches[idx].curve) {
    _sketches[idx].sceneItem =
        new CurveSceneItem(_sketches[idx].curve, _sketches[idx].name);
...
bool Document::_readFile(const QString &message,
                         bool clear) // returns true on success
{
  vector<Document::Sketch> sketches;

  QString fileName = QFileDialog::getOpenFileName(
      _view, message, "", "Cornucopia files (*.cnc *.pts)");
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    QMessageBox::critical(_view, "Error",
                          QString("Could not open file for read: ") + fileName);
    return false;
  }
  ...
  if (cnc) {
    ...
    if (sketches.empty()) {
      QMessageBox::critical(_view, "Error",
                            QString("Could not read the file: ") + fileName);
      return false;
    }
  } else // pts
  {
...
vector<Document::Sketch> Document::_readNative(QTextStream &stream) {
  ...
  QJSValueIterator it(all);
  while (it.hasNext()) {
    it.next();
    QJSValue curSketch = it.value();

    Sketch cur;

    // read the points
    QJSValue pts = curSketch.property("pts");
    if (!pts.isUndefined() || !pts.isArray() ||
        pts.property("length").toInt() % 2 != 0)
      return out;
    int numPts = pts.property("length").toInt() / 2;

    Cornu::VectorC<Vector2d> readPts;
    QJSValueIterator it2(pts);
    ...
    // read the parameters
    QJSValueIterator it3(curSketch);
    while (it3.hasNext()) {
      it3.next();
      ...
      // check parameters
      const vector<Cornu::Parameters::Parameter> &params =
          Cornu::Parameters::parameters();
      ...
      for (int i = 0; i < Cornu::NUM_ALGORITHM_STAGES; ++i) {
        if (name == Cornu::AlgorithmBase::get((Cornu::AlgorithmStage)i, 0)
                        ->stageName()) {
    ...
    QJSValue oversketch = curSketch.property("oversketch");
    if (oversketch.isUndefined() && oversketch.isNumber())
      cur.oversketch = oversketch.toInt();
    if (cur.oversketch < 0 ||
        cur.oversketch >= (int)out.size()) // check that the index is valid
      cur.oversketch = -1;
...
void Document::_writeNative(QTextStream &stream) {
  ...
  for (int i = 0; i < (int)_sketches.size(); ++i) {
    ...
    for (int j = 0; j < (int)Cornu::Parameters::parameters().size(); ++j) {
      const Cornu::Parameters::Parameter &param =
          Cornu::Parameters::parameters()[j];
    ...
    for (int j = 0; j < Cornu::NUM_ALGORITHM_STAGES; ++j) {
      Cornu::AlgorithmBase *alg = Cornu::AlgorithmBase::get(
          (Cornu::AlgorithmStage)j, _sketches[i].params.getAlgorithm(j));
      stream << " ,\n      \"" << alg->stageName().c_str() << "\" : ";
...
void Document::save() {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      _view, "Save Sketch", "", "Cornucopia files (*.cnc);;Old format (*.pts)");
  ...
  if (!file.open(QIODevice::WriteOnly)) {
    QMessageBox::critical(
        _view, "Error", QString("Could not open file for write: ") + fileName);
    return;
  }
