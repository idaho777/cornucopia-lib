...
Clothoid::Clothoid(const Vec &start, double startAngle, double length,
                   double curvature, double endCurvature) {
...
void Clothoid::_paramsChanged() {
  ...
  if (_arc) {
    ...
    } else // non-flat arc
    {
      ...
      double cosAS = cos(angle) / _params[CURVATURE],
             sinAS = sin(angle) / _params[CURVATURE];

      _mat << cosAS, -sinAS, sinAS, cosAS;
  ...
  } else // clothoid
  {
    ...
    if (_tdiff > 0) {
      ...
      double cosAS = PI * scale * cos(angleShift),
             sinAS = PI * scale * sin(angleShift);
      _mat << cosAS, -sinAS, sinAS, cosAS;
...
void Clothoid::eval(double s, Vec *pos, Vec *der, Vec *der2) const {
  ...
  if (der || der2) {
    double angle = _params[ANGLE] +
                   s * (_params[CURVATURE] + 0.5 * s * _params[DCURVATURE]);
    double cosa = cos(angle), sina = sin(angle);

    if (der)
      (*der) = Vector2d(cosa, sina);
    if (der2)
      (*der2) = (_params[CURVATURE] + s * _params[DCURVATURE]) *
                Vector2d(-sina, cosa);
  }
}
...
double Clothoid::project(const Vec &point) const {
  if (_flat) {
    ...
    // To compute the projection, get the angle difference into the range
    // centered on the midpoint of the arc.
    double t;
    if (_params[CURVATURE] > 0.) {
      double projAngleDiff = HALFPI + projAngle - _startAngle();
      t = AngleUtils::toRange(projAngleDiff, 0.5 * angleDiff - PI) /
          _params[CURVATURE];
    } else {
      double projAngleDiff = HALFPI - projAngle + _startAngle();
      t = AngleUtils::toRange(projAngleDiff, -0.5 * angleDiff - PI) /
  ...
  double bestT =
      _clothoidProjector()->project(pt, min(_t1, endT), max(_t1, endT));
...
void Clothoid::trim(double sFrom, double sTo) {
  ...
  _params[ANGLE] =
      angle(sFrom); // this line must come before the curvature change
  _params[CURVATURE] = curvature(
      sFrom); // the angle change in the previous line doesn't affect curvature
...
void Clothoid::derivativeAt(double s, ParamDer &out, ParamDer &outTan) const {
  ...
  if (_flat) {
    ...
    double curvs = curv * s;
    Vec curvDer(curvs * cosCur + sinStart - sinCur,
                curvs * sinCur + cosCur - cosStart);
    out.col(CURVATURE) = curvDer / (curv * curv);
    Vec dcurvDer(
        cosStart + (curvs * curvs * 0.5 - 1.) * cosCur - curvs * sinCur,
        sinStart + (curvs * curvs * 0.5 - 1.) * sinCur + curvs * cosCur);
    out.col(DCURVATURE) = dcurvDer / (curv * curv * curv);
  } else // non-degenerate
  {
    // This block is half-derived from Mathematica, and half from the
    // considerations below. Unfortunately it's a mess, but it works (see
    // CurveDerivativesTest). Any change to what we need to compute will require
    // rederivation. There's a bunch of optimizations possible, but they'll make
    // the code even worse.
    //
    ...
    double t = _t1 + s * _tdiff;
    double scale = sqrt(fabs(1. / (PI * _params[DCURVATURE])));
    RowVector2d dt1dx(scale,
                      -_params[CURVATURE] * scale / (2. * _params[DCURVATURE]));
    ...
    dmatdc << sinAS, cosAS, -cosAS, sinAS;
