...
public:
  ...
  template <class U>
  smart_ptr(const smart_ptr<U> &smartPtr)
      : ptr(smartPtr.ptr), typedPtr(smartPtr.typedPtr) {
    if (ptr)
      ptr->addRef();
  }
  ...
  template <class U> smart_ptr<T> &operator=(const smart_ptr<U> &other) {
  ...
  template <class U> bool operator<(const smart_ptr<U> &other) const {
...
template <class T> bool operator==(T *ptr1, const smart_ptr<T> &ptr2) {
