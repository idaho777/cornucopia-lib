...
public:
  ...
  PrimitiveCache(const Fitter &fitter, const FitPrimitive &primitive) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      int realEndIdx =
          primitive.isFixed() ? (pts.size() - 1) : primitive.endIdx;
      double endParam =
          (i == 0 ? curve->length() : curve->project(pts[realEndIdx - i]));
      _endData[i] = PrimitiveCacheData::make(curve, endParam);
    }
  }

  const PrimitiveCacheData &start(int offs) const {
    assert(offs < _numVals);
    return _startData[offs];
  }
  const PrimitiveCacheData &end(int offs) const {
    assert(offs < _numVals);
    return _endData[offs];
  }
...
public:
  CostEvaluator(const Fitter &fitter)
    ...
    for (int i = 0; i < 3; ++i) {
      _curveCost[i] = fitter.params().get(
          Parameters::ParameterType(i + Parameters::LINE_COST));
      _continuityCost[i] = fitter.params().get(
          Parameters::ParameterType(i + Parameters::G0_COST));
    }
    _errorCostFactor = fitter.params().get(Parameters::ERROR_COST);
    _inflectionCost = fitter.params().get(Parameters::INFLECTION_COST);
    _lengthScale = 1. / (fitter.output<SCALE_DETECTION>()->scale *
                         fitter.params().get(Parameters::PIXEL_SIZE));
    _shortnessCostFactor = fitter.params().get(Parameters::SHORTNESS_COST);
    _shortnessThreshold =
        fitter.scaledParameter(Parameters::SHORTNESS_THRESHOLD);
  ...
  double vertexCost(int p) const {
    ...
    if (_continuityCost[0] == Parameters::infinity) {
      // figure out how much we expect the length to decrease when we join
      // things up
  ...
  double edgeCost(int p1, int p2, int continuity, double error1 = -1.,
                  double error2 = -1.) const {
    double out = 0.;

    // continuity
    if (!_corners[_primitives[p1].endIdx]) // no primitive spans a corner, so
                                           // the start index is also there
      out += _continuityCost[continuity];

    // inflection
    if (continuity > 1 &&
        _primitives[p1].endCurvSign != _primitives[p2].startCurvSign)
      out += _inflectionCost;
...
private:
  ...
  void _getExtraError(int p1, int p2, int continuity, double &outExtra1,
                      double &outExtra2) const {
    ...
    outExtra1 =
        diffs[0] * 0.5 + len1 * diffs[1] * 0.25 + SQR(len1) * diffs[2] * 0.125;
    outExtra2 =
        diffs[0] * 0.5 + len2 * diffs[1] * 0.25 + SQR(len2) * diffs[2] * 0.125;
  }
  ...
  Vector3d _getDiffs(int p1, int p2, int offset) const {
    ...
    for (int i = 0; i <= offset; ++i) {
      ...
      // position
      minDistSq =
          min(minDistSq, Vector2d(d1.x - d2.x, d1.y - d2.y).squaredNorm());
    ...
    out[1] = (minAngleDiff * maxAngleDiff < 0.)
                 ? 0.
                 : min(fabs(minAngleDiff), fabs(maxAngleDiff));
    out[2] = (minCurvatureDiff * maxCurvatureDiff < 0.)
                 ? 0.
                 : min(fabs(minCurvatureDiff), fabs(maxCurvatureDiff));
...
protected:
  void _run(const Fitter &fitter, AlgorithmOutput<GRAPH_CONSTRUCTION> &out) {
    ...
    // Debugging::get()->printf("GraphConstructor: %d primitives",
    //                          primitives.size());
    for (FitPrimitive pr : primitives) {
      auto type = pr.curve->getType();
      int startidx = pr.startIdx;
      int endidx = pr.endIdx;
      int numPts = pr.numPts;
      double error = pr.error;
      // Debugging::get()->printf("Type: %d, Range [%d, %d], Num: %d, err: %f",
      //                          type, startidx, endidx, numPts, error);
    }
    ...
    for (int i = 0; i < (int)primitives.size(); ++i) {
      ...
      if (out.vertices[i].source &&
          out.vertices[i]
              .target) // one primitive over the entire curve--create dummy edge
      {
        Edge e;
        e.continuity = -1;
    ...
    for (int i = 0; i < (int)primitives.size(); ++i) {
      ...
      for (int continuity = 0; continuity <= 2; ++continuity) {
        ...
        bool firstCurveConstrained =
            (primitives[i].curve->getType() < continuity) ||
            primitives[i].isFixed();
        int minType = firstCurveConstrained ? continuity : 0;

        for (int j = 0; j < (int)curvesStartingAt[startIdx].size(); ++j) {
          int k = curvesStartingAt[startIdx][j]; // index of the second
                                                 // primitive
          int curve2len = primitives[k].numPts - 1;
          if (curve2len <= offset * 2)
            continue;

          bool secondCurveConstrained =
              (primitives[k].curve->getType() < continuity) ||
              primitives[k].isFixed();
          ...
          // Debugging::get()->printf("Edge1 %d %d continuity %d cost %f", i, k,
          //                          continuity, e.cost);
          e.cost +=
              out.vertices[i].cost * (out.vertices[i].source ? 1.f : 0.5f);
          // Debugging::get()->printf("Edge2 %d %d continuity %d cost %f", i, k,
          //                          continuity, e.cost);
          e.cost +=
              out.vertices[k].cost * (out.vertices[k].target ? 1.f : 0.5f);
          // Debugging::get()->printf("Edge3 %d %d continuity %d cost %f", i, k,
          //                          continuity, e.cost);
    ...
    Debugging::get()->printf("Graph vertices = %d edges = %d",
                             out.vertices.size(), out.edges.size());
  }
};
...
float Edge::validatedCost(const Fitter &fitter) const {
  ...
  float newCost;
  newCost = (float)fitter.output<GRAPH_CONSTRUCTION>()->costEvaluator->edgeCost(
      startVtx, endVtx, continuity, comb.err1, comb.err2);
