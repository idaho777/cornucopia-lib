...
NAMESPACE_Cornu
  ...
  PathFindingVertexData()
      : distance(0.), finished(false), prevEdge(-1), source(false),
        target(false), fixed(false), numIncoming(0), numOutgoing(0) {}
...
public:
  PathFindingGraph(const vector<Vertex> &vertices, const vector<Edge> &edges,
    ...
    const vector<FitPrimitive> &primitives =
        _fitter.output<PRIMITIVE_FITTING>()->primitives;
  ...
  vector<int> shortestPath() {
    ...
    int reduceEvery =
        max(1, (int)_fitter.params().get(Parameters::REDUCE_GRAPH_EVERY));
    vector<int> sp;
    ...
    Debugging::get()->printf("Found path, len = %d, cost = %lf", sp.size(),
                             total);
  ...
  vector<int> shortestCycle() {
    // start with the vertex that has an edge both cheap and with very connected
    // vertices
    ...
    for (size_t i = 0; i < _eData.size(); ++i) {
      double cost =
          _eData[i].cost() - double(_vData[_edges[i].startVtx].numIncoming) *
                                 _vData[_edges[i].endVtx].numOutgoing;
      if (cost < minEdgeCost) {
        minEdgeCost = cost;
        bestEdge = i;
      }
    ...
    for (int iter = 0; iter < 2; ++iter) {
      ...
      sources[0] = _edges[sp[sp.size() / 2]]
                       .endVtx; // the new source is the middlemost vertex

      // debugging output
      double total = 0;
      for (int j = 0; j < (int)sp.size(); ++j)
        total += _eData[sp[j]].cost();
      Debugging::get()->printf("Found cycle, len = %d, cost = %lf", sp.size(),
                               total);
    }
...
private:
  ...
  bool _validatePath(const vector<int> &path) {
    ...
    // line-clothoid-line
    if (valid) {
      int last = _fitter.output<CURVE_CLOSING>()->closed ? (int)path.size()
                                                         : (int)path.size() - 1;
      for (int i = 0; i < last; ++i) {
        int ni = (i + 1) % path.size();
        if (_edges[path[i]].continuity != 2 || _edges[path[ni]].continuity != 2)
          continue;
        if (!_vData[_edges[path[i]].startVtx].fixed &&
            _vData[_edges[path[i]].startVtx].primitiveType !=
                CurvePrimitive::LINE)
          continue;
        if (!_vData[_edges[path[ni]].endVtx].fixed &&
            _vData[_edges[path[ni]].endVtx].primitiveType !=
                CurvePrimitive::LINE)
          continue;
  ...
  void _reduceForPath(const vector<int> &sourceVertices) {
    ...
    for (int i = (int)_edges.size() - 1; i >= 0; --i) {
      ...
      _vData[src].distance =
          min(_vData[src].distance, _eData[i].cost() + _vData[tgt].distance);
    }
    ...
    for (int i = 0; i < (int)_edges.size(); ++i) {
      ...
      if (_vData[src].source)
        ...
        _eData[i].reduce(_vData[src].distance - _vData[tgt].distance -
                         reductionTol);
  ...
  void _reduceForCycle(int vertex) {
    ...
    for (int i = 0; i < (int)_edges.size(); ++i) {
      ...
      if (_vData[tgt].target) {
        ...
        if (!crossesSource &&
            _vData[src].distance <
                Parameters::infinity) // if the edge does not cross the starting
                                      // vertex
          _eData[i].reduce(_vData[src].distance - _vData[tgt].distance -
                           reductionTol);
        else
  ...
  vector<int> _shortestPath(const vector<int> &sourceVertices) {
    ...
    while (!todo.empty()) {
      ...
      if (_vData[v].target &&
          _vData[v].prevEdge >= 0) // done, now traverse the edges backwards
      {
        vector<int> out;
...
protected:
  void _run(const Fitter &fitter, AlgorithmOutput<PATH_FINDING> &out) {
    ...
    for (int i = 0; i < (int)shortestPath.size(); ++i) {
      ...
      ss << curveTypes[primitives[graph->edges[shortestPath[i]].startVtx]
                           .curve->getType()];
      ...
      if (!closed && i + 1 == (int)shortestPath.size())
        ss << curveTypes[primitives[graph->edges[shortestPath[i]].endVtx]
                             .curve->getType()];
    }
    Debugging::get()->printf("Curves = %s", ss.str().c_str());

    for (int i = 0; i < (int)shortestPath.size(); ++i) {
      Debugging::get()->drawPrimitive(
          primitives[graph->edges[shortestPath[i]].startVtx].curve, "Path", i);
    }
    if (shortestPath.size() > 0 &&
        graph->edges[shortestPath[0]].continuity != -1)
      Debugging::get()->drawPrimitive(
          primitives[graph->edges[shortestPath.back()].endVtx].curve, "Path",
...
void Algorithm<PATH_FINDING>::_initialize() { new DefaultPathFinder(); }
