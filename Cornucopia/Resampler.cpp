...
protected:
  // split the input into polylines based on corners and resample them
  // individually
  void _run(const Fitter &fitter, AlgorithmOutput<RESAMPLING> &out) {
    ...
    if (!poly->isClosed()) // if curve is not closed, start and end points are
                           // effectively corners
    ...
    if (!anyCorners) // also implies curve is closed
    {
      ...
      out.corners = VectorC<bool>(vector<bool>(out.output->pts().size(), false),
                                  CIRCULAR);
    ...
    for (int idx = 0; idx < pts.endIdx(1);
         ++idx) // go through all possible segment starting points
    {
      if (!corners[idx])
        continue;
      ...
      for (VectorC<Vector2d>::Circulator circ = pts.circulator(idx);;
           ++circ, ++idx) {
      ...
      vector<double> samples =
          _resample(fitter, segmentPoly, denseNearStart, denseNearEnd);
      PolylineConstPtr resampled = _processSamples(
          samples, segmentPoly, poly->idxToParam(segmentStartIdx), lengthSoFar,
          prevToCur);
      int startIdx = outputPts.empty() ? 0 : 1;
      copy(resampled->pts().begin() + startIdx, resampled->pts().end(),
           back_inserter(outputPts));
      out.corners.insert(out.corners.end(),
                         resampled->pts().end() -
                             (resampled->pts().begin() + startIdx),
                         false);
      out.corners[0] = out.corners.back() = true;
      lengthSoFar += resampled->length();

      --idx; // we should start the next segment from the last point of the
             // current one
    }
    ...
    // adjust the parameters into the range that the PiecewiseLinearMontone
    // object uses
  ...
  virtual vector<double> _resample(const Fitter &fitter, PolylineConstPtr poly,
                                   bool denseNearStart = false,
                                   bool denseNearEnd = false) = 0;

private:
  PolylineConstPtr _processSamples(const vector<double> &samples,
                                   PolylineConstPtr prev, double offsetPrev,
                                   double offsetCur,
                                   PiecewiseLinearMonotone &prevToCur) {
    ...
    if (prev->isClosed())
      prevToCur.add(offsetPrev + prev->length(),
                    offsetCur + lenSoFar + (out.back() - out[0]).norm());
  ...
  void displayOutput(AlgorithmOutput<RESAMPLING> &out, const Fitter &fitter) {
    for (int i = 0; i < out.output->pts().size(); ++i) {
      Debugging::get()->drawPoint(out.output->pts()[i],
                                  Vector3d(0, (i % 10 == 0) ? 0.6 : 0, 0),
                                  "Resampled");
      if (out.corners[i])
        Debugging::get()->drawPoint(out.output->pts()[i], Vector3d(0, 0, 0),
                                    "Resampled Corners");
    }
...
// Represents the piecewise-linear function over the parametric domain of the
// input curve whose value is an upper bound on the distance between samples.
class SampleSpacingFunction {
public:
  SampleSpacingFunction(PolylineConstPtr poly)
      : _values(vector<double>(poly->pts().size(), 0.), poly->pts().circular()),
        _maxSlope(1e10), _basePoly(poly) {
  ...
  double distToNext(int idx) {
    int flatIdx = _values.toLinearIdx(idx);
    return _lengths[flatIdx + 1] - _lengths[flatIdx];
  }
  ...
  void enforceMaxSlope(double maxSlope) {
    ...
    // Do this by walking forward and backward and enforcing this condition on
    // adjacent points--note that we need to loop around twice for closed curves
    // (well, technically, 1.5 times).
    int maxIdx = isClosed() ? (_values.size() * 2) : (_values.size() - 1);
    for (int i = 0; i < maxIdx; ++i) // forward
      _values[i + 1] =
          min(_values[i + 1], _values[i] + distToNext(i) * maxSlope);
  ...
  double eval(double s) const {
    ...
    return _values.flatAt(idx) +
           (cParam * invLength) * (_values.flatAt(nidx) - _values.flatAt(idx));
  }

  // Returns the maximum step we can take starting at s.  It's the side length
  // of the largest square we can inscribe under the function plot with a corner
  // at s.
  ...
  int paramToIdx(double param, double *outParam) const {
    int idx =
        (int)min(std::upper_bound(_lengths.begin(), _lengths.end(), param) -
                     _lengths.begin(),
                 (ptrdiff_t)_lengths.size() - 1) -
        1;
    if (outParam)
  ...
  SampleSpacingFunction trimmed(double from, double to) {
    // Trim the base polyline and evaluate the function on it.  Not the fastest
  ...
  void draw() {
    ...
    static int cnt = 0;
    ++cnt;
    snprintf(name, sizeof(name), "Func %d", cnt);
    Vector2d offs(10, 20 + 50 * cnt);
    Debugging::get()->drawLine(offs, offs + Vector2d(_lengths.back(), 0),
                               Vector3d(0, 0, 0), name);
    for (int i = 0; i < _values.endIdx(1); ++i)
      Debugging::get()->drawLine(offs + Vector2d(_lengths[i], -_values[i]),
                                 offs +
                                     Vector2d(_lengths[i + 1], -_values[i + 1]),
                                 Vector3d(1, 0, 0), name);
    ...
    while (param < _lengths.back()) {
      ...
      Debugging::get()->drawLine(corner, corner + Vector2d(0, -step),
                                 Vector3d(0, 1, 0), name);
      Debugging::get()->drawLine(corner + Vector2d(0, -step),
                                 corner + Vector2d(step, -step),
                                 Vector3d(0, 1, 0), name);
      Debugging::get()->drawLine(corner + Vector2d(step, -step),
                                 corner + Vector2d(step, 0), Vector3d(0, 1, 0),
                                 name);
      param += step;
    }
  }
...
protected:
  vector<double> _resample(const Fitter &fitter, PolylineConstPtr poly,
                           bool denseNearStart = false,
                           bool denseNearEnd = false) {
    ...
    double regionSize =
        fitter.scaledParameter(Parameters::CURVATURE_ESTIMATE_REGION);
    double maxInterval =
        fitter.scaledParameter(Parameters::MAX_SAMPLING_INTERVAL);
    double pointsPerCircle = fitter.params().get(Parameters::POINTS_PER_CIRCLE);
    double arcFitterScale =
        1. /
        fitter.scale(); // our ArcFitter is approximate and not scale-invariant,
                        // so we scale its input and output

    for (int i = 0; i < pts.size(); ++i) {
      ArcFitter arcFitter;
      // step forward
      double startParam = poly->idxToParam(i);
      for (double param = startParam;
           poly->isParamValid(param) && param < startParam + regionSize;
           param += step)
      ...
      double curvature =
          fabs(arcFitter.getCurve()->startCurvature()) * arcFitterScale;
      spacing.values()[i] =
          TWOPI / max(pointsPerCircle * curvature, TWOPI / maxInterval);
    ...
    // We need for the last sample to end precisely at the end of the curve.
    // This won't happen naturally, so we scale the sampling rate until it does.
    // Start by figuring out how many samples we'll have and what the
    // "remainder" is and an initial guess at how much we need to scale.
    int numPts = 0;
    double param = 0;
    while (true) {
    ...
    // We maintain the sampling spacing scales and the ratio of the last
    // parameter to the curve length as a piecewise linear function. It is
    // guaranteed to be monotone--reducing the sampling spacing must reduce the
    // last parameter.
    ...
    for (int iters = 0; iters < 3;
         ++iters) // three iterations have been sufficient for convergence
    {
...
#if RESAMPLING_DEBUG
    // DBG:
    Debugging::get()->printf("Log scale error Final = %lf",
                             log(poly->length() / param));
    spacing.draw();
#endif
