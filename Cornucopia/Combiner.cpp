...
public:
  ...
  void solveForDelta(double damping, Eigen::VectorXd &out,
                     std::set<LSBoxConstraint> &constraints) {
    ...
    int cnt = 0;
    for (set<LSBoxConstraint>::const_iterator it = constraints.begin();
         it != constraints.end(); ++it, ++cnt)
      lhs(vars + _conDer.rows() + cnt, it->index) =
          lhs(it->index, vars + _conDer.rows() + cnt) = 1.;
    ...
    cnt = 0;
    for (set<LSBoxConstraint>::iterator it = constraints.begin();
         it != constraints.end(); ++cnt) {
...
public:
  ...
  typedef vector<BlockCholType, aligned_allocator<BlockCholType>>
      BlockCholVectorType;
  ...
  void solveForDelta(double damping, Eigen::VectorXd &out,
    ...
    for (size_t i = 0; i < cholBlocks.size(); ++i)
      cholBlocks[i] = BlockCholType(
          _errDerBlocks[i] +
          damping * MatrixXd::Identity(_blockSizes[i], _blockSizes[i]));
    ...
    mid.segment(vars, cons) = CholK.matrixL().solve(
        rhs.segment(vars, cons) - K.adjoint() * mid.segment(0, vars));

    // solve for result
    result.segment(vars, cons) =
        -CholK.matrixU().solve(mid.segment(vars, cons));
    _solveCholU(result.segment(0, vars), cholBlocks,
                mid.segment(0, vars) - K * result.segment(vars, cons));
...
private:
  ...
  MatrixXd _blocksToMatrix(const BlockVectorType &blocks) {
    ...
    for (int i = 0; i < (int)blocks.size(); ++i)
      out.block(_blockIndices[i], _blockIndices[i], _blockSizes[i],
                _blockSizes[i]) = blocks[i];

    return out;
  }

  template <class M1, class M2>
  void _solveCholL(const M1 &out, const BlockCholVectorType &chols,
                   const M2 &rhs) {
    for (int i = 0; i < (int)chols.size(); ++i)
      const_cast<M1 &>(out).block(_blockIndices[i], 0, _blockSizes[i],
                                  out.cols()) =
          chols[i].matrixL().solve(
              rhs.block(_blockIndices[i], 0, _blockSizes[i], rhs.cols()));
  }

  template <class M1, class M2>
  void _solveCholU(const M1 &out, const BlockCholVectorType &chols,
                   const M2 &rhs) {
    for (int i = 0; i < (int)chols.size(); ++i)
      const_cast<M1 &>(out).block(_blockIndices[i], 0, _blockSizes[i],
                                  out.cols()) =
          chols[i].matrixU().solve(
              rhs.block(_blockIndices[i], 0, _blockSizes[i], rhs.cols()));
  }
...
public:
  MulticurveProblem(const Fitter &fitter)
    ...
    _inflectionAccounting =
        fitter.params().get(Parameters::INFLECTION_COST) > 0.;
    ...
    _curves = VectorC<CurvePrimitivePtr>((int)_primIdcs.size(),
                                         _closed ? CIRCULAR : NOT_CIRCULAR);
    _curveRanges =
        VectorC<pair<int, int>>((int)_primIdcs.size(), _curves.circular());
    for (int i = 0; i < (int)_primIdcs.size(); ++i) {
      _curveRanges[i] = make_pair(_primitives[_primIdcs[i]].startIdx,
  ...
  vector<LSBoxConstraint> getConstraints() const {
    ...
    for (int i = 0; i < _curves.size(); ++i) {
      ...
      out.push_back(LSBoxConstraint(curVar + CurvePrimitive::LENGTH,
                                    _curves[i]->length() * 0.5, 1));

      // inflections
      // Debugging::get()->printf("idx = %d Type = %d, startSign = %d, endSign =
      // %d", i, _curves[i]->getType(), _primitives[primIdx].startCurvSign,
      // _primitives[primIdx].endCurvSign);
      if (_inflectionAccounting &&
          _curves[i]->getType() >= CurvePrimitive::ARC) {
        ...
        bool line2Prev =
            c2toPrev && _curves[pi]->getType() == CurvePrimitive::LINE;
        bool line2Next =
            c2toNext && _curves[ni]->getType() == CurvePrimitive::LINE;

        int startSign = _primitives[primIdx].startCurvSign;
        int endSign = _primitives[primIdx].endCurvSign;

        if (!c2toPrev || (endSign == _primitives[_primIdcs[pi]].startCurvSign &&
                          startSign == endSign && !line2Prev))
          out.push_back(LSBoxConstraint(curVar + CurvePrimitive::CURVATURE, 0.,
                                        startSign));

        if (c2toPrev && c2toNext &&
            startSign != endSign) // constrain the "easier one" of the endpoints
        {
          ...
          if (startViolation < endViolation)
            out.push_back(LSBoxConstraint(curVar + CurvePrimitive::CURVATURE,
                                          0., startSign));
          else
            out.push_back(LSBoxConstraint(curVar + CurvePrimitive::DCURVATURE,
                                          0., endSign));
        }

        if (!c2toNext && _curves[i]->getType() == CurvePrimitive::CLOTHOID)
          out.push_back(LSBoxConstraint(curVar + CurvePrimitive::DCURVATURE, 0.,
                                        endSign));
      }
  ...
  void setParams(const Eigen::VectorXd &x) {
    ...
    for (int i = 0; i < (int)_curves.size(); ++i) {
      if (_curves[i]->getType() != CurvePrimitive::CLOTHOID) {
        ...
        xm(CurvePrimitive::DCURVATURE) =
            (xm(CurvePrimitive::DCURVATURE) - xm(CurvePrimitive::CURVATURE)) /
            xm(CurvePrimitive::LENGTH);
    ...
    if (false) {
      ...
      snprintf(name, sizeof(name), "Out%d", _iter);
      for (int i = 0; i < _curves.size(); ++i)
  ...
  VectorXd params() const {
    ...
    for (int i = 0; i < (int)_curves.size(); ++i) {
      if (_curves[i]->getType() != CurvePrimitive::CLOTHOID) {
        ...
        xm(CurvePrimitive::DCURVATURE) =
            xm(CurvePrimitive::CURVATURE) +
            xm(CurvePrimitive::DCURVATURE) * xm(CurvePrimitive::LENGTH);
  ...
  double objective() const {
    ...
    for (int i = 0; i < (int)_curves.size(); ++i) {
      ...
      bool firstCorner =
          (!_closed && i == 0) || (_continuities[(i + csz - 1) % csz] == 0);
      bool lastCorner =
          (!_closed && i + 1 == (int)_curves.size()) || (_continuities[i] == 0);

      out += _errorComputer->computeError(_curves[i], _curveRanges[i].first,
                                          _curveRanges[i].second, firstCorner,
                                          lastCorner);
    }
...
private:
  void _evalError(EvalDataType *evalData) {
    ...
    for (int i = 0; i < (int)_curves.size(); ++i) {
      ...
      _errorComputer->computeErrorVector(
          _curves[i], _curveRanges[i].first, _curveRanges[i].second, errVecs[i],
          &(errVecDers[i]), firstCorner, lastCorner);
...
#if SPARSE
    EvalDataType::BlockVectorType &outErrDerBlocks =
        evalData->errDerBlocksRef();
  ...
  void _evalConstraints(EvalDataType *evalData) {
    ...
    for (int i = 0; i < (int)_continuities.size(); ++i) {
      ...
      if (_continuities[i] >= 1)
        conVecs[i][2] = AngleUtils::toRange(
            _curves[i]->endAngle() - _curves[i + 1]->startAngle(), -PI);
      if (_continuities[i] == 2)
        conVecs[i][3] =
            _curves[i]->endCurvature() - _curves[i + 1]->startCurvature();
    ...
    for (int i = 0; i < (int)_continuities.size(); ++i) {
      ...
      if (nCon > 2)
        outConDer(curCon + 2,
                  (curVar + nVar + CurvePrimitive::ANGLE) % numVar) = -1.;
      if (nCon > 3 && _curves[i + 1]->getType() != CurvePrimitive::LINE)
        outConDer(curCon + 3,
                  (curVar + nVar + CurvePrimitive::CURVATURE) % numVar) = -1.;
...
protected:
  void _run(const Fitter &fitter, AlgorithmOutput<COMBINING> &out) {
    ...
    } else // solve the nonlinear problem
    {
      ...
      solver.setDefaultDamping(
          fitter.params().get(Parameters::COMBINE_DAMPING));
      ...
      Debugging::get()->printf("Final objective = %lf",
                               sqrt(problem.objective()));
    ...
    int minParamSample =
        min_element(idxToParam.begin(), idxToParam.end()) - idxToParam.begin();
    ...
    lenSoFar = 0;
    for (VectorC<Vector2d>::Circulator ci =
             resampled.circulator(minParamSample + 1);
         !ci.done(); ++ci) {
      ...
      if (ci.index() == minParamSample)
        ...
        idxToParam[ci.index()] =
            max(idxToParam[ci.index()],
                idxToParam[(ci - 1).index()]); // ensure monotonicity
    ...
    smart_ptr<const AlgorithmOutput<OVERSKETCHING>> osOutput =
        fitter.output<OVERSKETCHING>();
    VectorC<CurvePrimitiveConstPtr> outFinal(
        0, osOutput->finallyClose ? CIRCULAR : NOT_CIRCULAR);

    if (osOutput->toPrepend) {
      outFinal.insert(outFinal.end(), osOutput->toPrepend->primitives().begin(),
                      osOutput->toPrepend->primitives().end() - 1);
      for (int i = 0; i < (int)out.parameters.size(); ++i)
        out.parameters[i] +=
            (osOutput->toPrepend->length() -
             osOutput->toPrepend->primitives().back()->length());
    }
    outFinal.insert(outFinal.end(), outV.begin(), outV.end());

    if (osOutput->toAppend) {
      if (!osOutput->finallyClose) {
        outFinal.insert(outFinal.end(),
                        osOutput->toAppend->primitives().begin() + 1,
                        osOutput->toAppend->primitives().end());
      } else {
        if (osOutput->toAppend->primitives().size() >= 2) {
          outFinal.insert(outFinal.end(),
                          osOutput->toAppend->primitives().begin() + 1,
                          osOutput->toAppend->primitives().end() - 1);
        } else // start and end curve is the same curve -- its original length
               // is the one toAppend curve
        {
          ...
          outFinal[0] =
              outFinal[0]->trimmed(origLen - lastCurveLen, firstCurveLen);
          for (int i = 0; i < (int)out.parameters.size(); ++i)
    ...
    for (int i = 0; i < (int)out.parameters.size(); ++i) {
      ...
      Debugging::get()->drawLine(fitter.originalSketch()->pts()[i],
                                 out.output->pos(out.parameters[i]),
                                 Vector3d(1, 0, 1), "Correspondence");
    }
#endif
  }
};

void Algorithm<COMBINING>::_initialize() { new DefaultCombiner(); }
