...
public:
  ...
  void setParams(const Parameters &params) {
    _params = params;
    _clearBefore(SCALE_DETECTION);
  }

  PolylineConstPtr originalSketch() const { return _originalSketch; }
  void setOriginalSketch(PolylineConstPtr originalSketch) {
    _originalSketch = originalSketch;
    _clearBefore(SCALE_DETECTION);
  }

  PrimitiveSequenceConstPtr oversketchBase() const { return _oversketchBase; }
  void setOversketchBase(PrimitiveSequenceConstPtr oversketchBase) {
    _oversketchBase = oversketchBase;
    _clearBefore(SCALE_DETECTION);
  }

  template <int AlgStage>
  smart_ptr<const AlgorithmOutput<AlgStage>> output() const {
    return static_pointer_cast<const AlgorithmOutput<AlgStage>>(
        _outputs[AlgStage]);
  }

  void run();

  PrimitiveSequenceConstPtr
  finalOutput() const; // returns null if fitting failed for some reason
  const std::vector<double> &originalSketchToFinalParameters()
      const; // returns a vector that for each original sketch point has the
