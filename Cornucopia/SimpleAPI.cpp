...
NAMESPACE_Cornu
    ...
    vector<BasicPrimitive>
    fit(const vector<Point> &points, const Parameters &parameters,
        bool *outClosed) {
  Fitter fitter;
...
CurvePrimitivePtr _toCurvePrimitive(const BasicPrimitive &primitive) {
  ...
  case BasicPrimitive::ARC:
    ...
    break;
  case BasicPrimitive::ANTICLOTHOID:
    out = new Clothoid();
    params[CurvePrimitive::CURVATURE] = primitive.startCurvature;
...
void BasicPrimitive::eval(double s, Point *outPos, Point *outDer,
                          Point *outDer2) const {
...
vector<BasicBezier> toBezierSpline(const vector<BasicPrimitive> &curve,
                                   double tolerance) {
