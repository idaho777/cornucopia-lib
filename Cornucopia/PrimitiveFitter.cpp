...
public:
  OneCurveProblem(const FitPrimitive &primitive,
  ...
  double error(const VectorXd &x, LSEvalData *) {
    setParams(x);
    return _errorComputer->computeError(_primitive.curve, _primitive.startIdx,
                                        _primitive.endIdx);
  }
  ...
  void eval(const VectorXd &x, LSEvalData *data) {
    ...
    _errorComputer->computeErrorVector(_primitive.curve, _primitive.startIdx,
                                       _primitive.endIdx,
  ...
  VectorXd params() const {
    ...
    out(CurvePrimitive::DCURVATURE) =
        out(CurvePrimitive::CURVATURE) +
        out(CurvePrimitive::LENGTH) * out(CurvePrimitive::DCURVATURE);
    return out;
  }
...
protected:
  void _run(const Fitter &fitter, AlgorithmOutput<PRIMITIVE_FITTING> &out) {
    ...
    ErrorComputerConstPtr errorComputer =
        fitter.output<ERROR_COMPUTER>()->errorComputer;
    ...
    const double errorThreshold =
        fitter.scaledParameter(Parameters::ERROR_THRESHOLD);
    // std::string typeNames[3] = {"Lines", "Arcs", "Clothoids"};
    std::string typeNames[3] = {"Lines", "Arcs", "Anticlothoids"};
    bool inflectionAccounting =
    ...
    if (osOutput->startCurve) {
      ...
      for (int i = 1; i < pts.size(); ++i) {
        ...
        fit.curve->trim(0,
                        fit.curve->length() + 2 * (pts[i] - pts[i - 1]).norm());
        fit.curve->trim(0, fit.curve->project(pts[i]));

        fit.endCurvSign = (fit.curve->endCurvature() >= 0) ? 1 : -1;
        fit.error =
            errorComputer->computeErrorForCost(fit.curve, 0, fit.endIdx, false);
    ...
    if (osOutput->endCurve) {
      ...
      for (int i = fit.startIdx - 1; i >= 0; --i) {
        ...
        fit.startCurvSign = (fit.curve->startCurvature() >= 0) ? 1 : -1;
        fit.error = errorComputer->computeErrorForCost(
            fit.curve, fit.startIdx, (int)pts.size() - 1, true, false);
    ...
    {
      int total_types = 3;
      FitterBasePtr fitters[total_types];
      fitters[0] = new LineFitter();
      fitters[1] = new ArcFitter();
      // fitters[2] = new ClothoidFitter();
      fitters[2] = new AnticlothoidFitter();

      // iterate over lines, arcs, clothoids
      // for (int type = 0; type < total_types; ++type) {
      for (int type = 2; type < total_types; ++type) {
        int fitSoFar = 0;

        bool needType =
            fitter.params().get(Parameters::ParameterType(
                Parameters::LINE_COST + type)) < Parameters::infinity;

        for (VectorC<Vector2d>::Circulator circ = pts.circulator(i);
             !circ.done(); ++circ) {
          ++fitSoFar;
          ...
          if (!needType &&
              (type == 2 ||
               fitSoFar >=
                   3 + type)) // if we don't need primitives of this type
            break;
          ...
          if (fitSoFar >= 2 + type) // at least two points per line, etc.
          {
            ...
            fit.error =
                errorComputer->computeErrorForCost(curve, i, fit.endIdx);
            ...
            if (type == 0 &&
                inflectionAccounting) // line with "opposite" curvature
            {
            ...
            if (fit.startCurvSign != fit.endCurvSign && inflectionAccounting &&
              ...
              CurvePrimitivePtr startNoCurv =
                  static_pointer_cast<ClothoidFitter>(fitters[2])
                      ->getCurveWithZeroCurvature(0);
              CurvePrimitivePtr endNoCurv =
                  static_pointer_cast<ClothoidFitter>(fitters[2])
                      ->getCurveWithZeroCurvature(end - start);

              // CurvePrimitivePtr startNoCurv =
              //     static_pointer_cast<AnticlothoidFitter>(fitters[2])
              //         ->getCurveWithZeroCurvature(0);
              // CurvePrimitivePtr endNoCurv =
              //     static_pointer_cast<AnticlothoidFitter>(fitters[2])
              //         ->getCurveWithZeroCurvature(end - start);
              ...
              fit.startCurvSign = fit.endCurvSign =
                  (startNoCurv->endCurvature() > 0. ? 1 : -1);

              if (_adjust)
                adjustPrimitive(fit, fitter);

              fit.error =
                  errorComputer->computeErrorForCost(fit.curve, i, fit.endIdx);

              if (fit.error < errorThreshold * errorThreshold) {
                out.primitives.push_back(fit);
                // Debugging::get()->drawCurve(fit.curve, color,
                // typeNames[type]);
              }

              fit.curve = endNoCurv;
              fit.startCurvSign = fit.endCurvSign =
                  (endNoCurv->startCurvature() > 0. ? 1 : -1);
  ...
  void adjustPrimitive(const FitPrimitive &primitive, const Fitter &fitter) {
    ...
    constraints.push_back(LSBoxConstraint(CurvePrimitive::LENGTH,
                                          primitive.curve->length() * 0.5, 1));

    // curvature sign constraints
    if (inflectionAccounting) {
      if (primitive.curve->getType() >= CurvePrimitive::ARC)
        constraints.push_back(LSBoxConstraint(CurvePrimitive::CURVATURE, 0.,
                                              primitive.startCurvSign));
      if (primitive.curve->getType() == CurvePrimitive::CLOTHOID)
        constraints.push_back(LSBoxConstraint(CurvePrimitive::DCURVATURE, 0.,
                                              primitive.endCurvSign));
    }
    ...
    solver.setDefaultDamping(
        fitter.params().get(Parameters::CURVE_ADJUST_DAMPING));
    solver.setMaxIter(1);
