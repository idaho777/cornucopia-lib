...
LSSolver::LSSolver(LSProblem *problem,
                   const vector<LSBoxConstraint> &constraints)
...
VectorXd LSSolver::solve(const VectorXd &guess) {
  ...
  if (iter > 5)
    Debugging::get()->printf("After %d iterations, error = %lf", iter,
                             sqrt(error));
  if (error < bestError) {
    best = x;
  }
...
int LSSolver::_project(const VectorXd &from, VectorXd &delta,
                       const set<LSBoxConstraint> &activeSet) {
  ...
  if (closestConstraint >= 0) {
    delta *= minScale;
    // cout << "Projecting up to constraint " << closestConstraint << " by " <<
    // minScale << endl;
  }
...
void LSDenseEvalData::solveForDelta(double damping, VectorXd &out,
                                    set<LSBoxConstraint> &constraints) {
  int vars = (int)_errDer.cols();
  if (constraints.empty()) {
    LDLT<MatrixXd> ldlt(MatrixXd::Identity(vars, vars) * damping +
                        _errDer.transpose() * _errDer);
    ...
    if (vars > (int)constraints.size()) {
      ...
      LDLT<MatrixXd> ldlt(MatrixXd::Identity(lhs.cols(), lhs.cols()) * damping +
                          lhs.transpose() * lhs);
    ...
    for (set<LSBoxConstraint>::iterator it = constraints.begin();
      ...
      ++next;
      if (gradient[it->index] * it->sign <
          0) // if sign is zero, constraint will not get erased
      {
        // cout << "Unsetting constraint on variable at index " << it->index <<
        // endl;
