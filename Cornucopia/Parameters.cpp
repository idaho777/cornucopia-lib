...
void Parameters::_initializeParameters() {
  ...
  _parameters.push_back(
      Parameter(CLOTHOID_COST, "Clothoid cost", 0., 50., 15.));
  ...
  _parameters.push_back(
      Parameter(SHORTNESS_COST, "Shortness cost", 0., 10., 2.));
  _parameters.push_back(
      Parameter(INFLECTION_COST, "Inflection cost", 0., 100., 20.));

  _parameters.push_back(
      Parameter(INTERNAL_PARAMETERS_MARKER, "NOT A PARAMETER", 0., 0., 0.));
  _parameters.push_back(Parameter(PIXEL_SIZE, "Pixel size", 1.));
  _parameters.push_back(
      Parameter(SMALL_CURVE_PIXELS, "Small curve pixels", 200.));
  _parameters.push_back(
      Parameter(LARGE_CURVE_PIXELS, "Large curve pixels", 500.));
  ...
  _parameters.push_back(
      Parameter(CLOSEDNESS_THRESHOLD, "Closedness threshold", 20.));
  _parameters.push_back(
      Parameter(MINIMUM_CORNER_SPACING, "Min corner spacing", 5.));
  _parameters.push_back(
      Parameter(CORNER_NEIGHBORHOOD, "Corner neighborhood", 15.));
  _parameters.push_back(
      Parameter(DENSE_SAMPLING_STEP, "Dense sampling step", 1.));
  _parameters.push_back(
      Parameter(CORNER_SCALES, "Num corner scales (int)", 5.));
  _parameters.push_back(
      Parameter(CORNER_THRESHOLD, "Corner angle threshold", PI * 0.25));
  _parameters.push_back(
      Parameter(MAX_SAMPLING_INTERVAL, "Maximum sampling interval", 50.));
  _parameters.push_back(
      Parameter(CURVATURE_ESTIMATE_REGION, "Curvature estimate region", 20.));
  _parameters.push_back(Parameter(POINTS_PER_CIRCLE, "Points per circle", 20.));
  _parameters.push_back(
      Parameter(MAX_SAMPLE_RATE_SLOPE, "Max sample rate slope", 0.4));
  _parameters.push_back(Parameter(ERROR_THRESHOLD, "Error Threshold", 4.));
  _parameters.push_back(
      Parameter(SHORTNESS_THRESHOLD, "Shortness Threshold", 50.));
  _parameters.push_back(
      Parameter(TWO_CURVE_CURVATURE_ADJUST, "Two-Curve Adjustment Point", 2.));
  _parameters.push_back(
      Parameter(CURVE_ADJUST_DAMPING, "Curve Adjust Damping", 1.));
  _parameters.push_back(
      Parameter(REDUCE_GRAPH_EVERY, "Reduce Graph Every", 10.));
  _parameters.push_back(Parameter(COMBINE_DAMPING, "Combine Damping", 2.));
  _parameters.push_back(
      Parameter(OVERSKETCH_THRESHOLD, "Oversketch Threshold", 15.));
}
